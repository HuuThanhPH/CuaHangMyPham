...
namespace MongoWeb.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult QuanLyDonHang()
        {
            ...
            {
                TempData["ErrorMessage"] = $"Failed to retrieve orders: {ex.Message}";
                return View(new List<Order>());
            }
        }

        [HttpPost]
        public ActionResult UpdateOrderStatus(Dictionary<string, string> orderStatuses, Dictionary<string, string> orderIds)
        {
            var errorMessages = new List<string>();

            foreach (var orderId in orderIds.Values)
            {
                var status = orderStatuses[orderId];

                try
                {
                    _repository.UpdateStatus(orderId, new Order { Status = status });
                }
                catch (Exception ex)
                {
                    errorMessages.Add($"Failed to update order {orderId}: {ex.Message}");
                }
            }
